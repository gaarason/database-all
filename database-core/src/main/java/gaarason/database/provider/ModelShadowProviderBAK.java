...
// * @author xt
// */
//public class ModelShadowProviderBAK extends Container.SimpleKeeper {
//
//    /**
//     * Model Class做为索引
//     */
//    private final Map<Class<? extends Model<? extends Serializable, ? extends Serializable>>, ModelInfo<? extends Serializable, ? extends Serializable>> MODEL_INDEX_MAP = new ConcurrentHashMap<>();
//
//    /**
//     * Model proxy Class做为索引
//     */
//    private final Map<Class<?>, ModelInfo<? extends Serializable, ? extends Serializable>> MODEL_PROXY_INDEX_MAP = new ConcurrentHashMap<>();
//
//    /**
//     * table name 作为索引
//     */
//    private final Map<String, ModelInfo<? extends Serializable, ? extends Serializable>> TABLE_NAME_INDEX_MAP = new ConcurrentHashMap<>();
//
//    /**
//     * Entity Class作为索引
//     */
//    private final Map<Class<? extends Serializable>, ModelInfo<? extends Serializable, ? extends Serializable>> ENTITY_INDEX_MAP = new ConcurrentHashMap<>();
//
//    /**
//     * Entity 子类 Class作为索引
//     */
//    private final SoftCache<Class<?>, ModelInfo<? extends Serializable, ? extends Serializable>> ENTITY_SUBCLASS_MODEL_INFO_CACHE = new SoftCache<>();
//
//    /**
//     * 缓存lambda风格的列名, 与为String风格的列名的映射
//     */
//    private final SoftCache<Class<?>, String> LAMBDA_COLUMN_NAME_CACHE = new SoftCache<>();
//
//    /**
//     * 缓存lambda风格的属性名, 与为String风格的属性名的映射
//     */
//    private final SoftCache<Class<?>, String> LAMBDA_FIELD_NAME_CACHE = new SoftCache<>();
//
//    public ModelShadowProviderBAK(Container container) {
//        super(container);
//    }
//
//    /**
...
//     */
//    public synchronized void loadModels(Collection<Class<? extends Model<?, ?>>> modelClasses) {
//        /*
//         * Model Class做为索引
//         */
...
//        // 初始化模型的基本信息, 并构建索引
//        initModelInformation(modelClasses, localEntityIndexMap, localModelIndexMap,
//            localTableNameIndexMap);
//
//        // 二轮补充基础字段信息
//        // Model实例化存储
...
//     * @param modelClasses model类型的集合
//     */
//    public void refreshModels(Collection<Class<? extends Model<?, ?>>> modelClasses) {
...
//     * @param modelClasses model类型的集合
//     */
//    public synchronized void unloadModels(Collection<Class<? extends Model<?, ?>>> modelClasses) {
//        for (Class<? extends Model<?, ?>> modelClass : modelClasses) {
//            ModelInfo<? extends Serializable, ? extends Serializable> modelInfo = MODEL_INDEX_MAP.remove(modelClass);
//            if (!ObjectUtils.isEmpty(modelInfo)) {
//                ENTITY_INDEX_MAP.remove(modelInfo.entityClass);
//                TABLE_NAME_INDEX_MAP.remove(modelInfo.tableName);
//                MODEL_PROXY_INDEX_MAP.entrySet().removeIf(entry -> entry.getValue() == modelInfo);
//            }
//        }
//    }
//
...
//     * @return 格式化后的Model信息
//     */
//    public <T extends Serializable, K extends Serializable> ModelInfo<T, K> get(Model<T, K> model) {
...
//     * @return 格式化后的Model信息
//     */
//    public <T extends Serializable, K extends Serializable> ModelInfo<T, K> getByModelClass(
...
//     * @return 格式化后的Model信息
//     */
//    public ModelInfo<?, ?> getByEntityClass(Class<?> clazz) {
//        ModelInfo<?, ?> result;
//        result = ENTITY_INDEX_MAP.get(clazz);
//        if (null == result) {
//            result = ENTITY_SUBCLASS_MODEL_INFO_CACHE.get(clazz);
//            if (null == result) {
//                synchronized (clazz.getName()) {
//                    // 递归查找其父类
//                    Class<?> superclass = clazz.getSuperclass();
//                    if (superclass != null) {
//                        result = getByEntityClass(superclass);
//
//                        // 软引用缓存
//                        ENTITY_SUBCLASS_MODEL_INFO_CACHE.put(clazz, result);
//                    }
//                }
//            }
//        }
...
//     * @return 格式化后的Model信息
//     */
//    public <T extends Serializable> ModelInfo<T, Serializable> getByTableName(String tableName) {
...
//     * @return 列名
//     */
//    public <T extends Serializable> String getFieldNameByLambdaWithCache(ColumnFunctionalInterface<T> func) {
...
//     * @return 列名
//     */
//    public <T extends Serializable> String getColumnNameByLambdaWithCache(ColumnFunctionalInterface<T> func) {
//        Class<?> clazz = func.getClass();
...
//     */
//    public <T extends Serializable> FieldInfo getFieldInfoByEntityClass(Class<T> clazz, String fieldName)
//        throws EntityAttributeInvalidException, EntityInvalidException {
//        final ModelInfo<?, ?> modelInfo = getByEntityClass(clazz);
...
//     * @return 字段对值的映射
//     */
//    public <T extends Serializable> Map<String, Object> columnValueMap(@Nullable T entity) {
//        // 结果集
...
//                columnValueMap.put(entry.getKey(), valueFormat(value));
//            }
//        }
//        return columnValueMap;
//    }
...
//     * @return 字段对值的映射
//     */
//    public <T extends Serializable> Map<String, Object> columnValueMap(@Nullable T entity, boolean insertType) {
//        // 结果集
//        Map<String, Object> columnValueMap = new HashMap<>();
//        if (ObjectUtils.isNull(entity)) {
//            return columnValueMap;
...
//     * @return 列名组成的list
//     */
//    public <T extends Serializable> List<String> columnNameList(T entity, boolean insertType) {
//        // 结果集
...
//     * @return 列名组成的set
//     */
//    public <T extends Serializable> Set<String> columnNameSet(T entity, boolean insertType) {
...
//     * @return 字段的值组成的list
//     */
//    public <T extends Serializable> List<Object> valueList(T entity, Collection<String> columnNames) {
//        // 结果集
...
//                valueList.add(valueFormat(fieldGet(fieldInfo, entity)));
//            }
//        }
//        return valueList;
//    }
...
//        Map<String, gaarason.database.appointment.Column> stringColumnMap, T entity, Record<T, K> theRecord)
...
//     */
//    public <T extends Serializable, K extends Serializable> void setPrimaryKeyValue(T entity, @Nullable K id) {
//        // 属性信息集合
...
//     */
//    @Nullable
//    public <T extends Serializable, K extends Serializable> K getPrimaryKeyValue(T entity) {
//        // 属性信息集合
//        FieldInfo primaryKeyFieldInfo = getByEntityClass(entity.getClass()).getPrimaryKeyFieldInfo();
//        if (null != primaryKeyFieldInfo) {
...
//        Collection<Class<? extends Model<?, ?>>> modelClasses,
...
//            modelInfo.modelClass = ObjectUtils.typeCast(modelClass);
//            try {
//                // 模型信息 设置详情信息
//                modelDeal(modelInfo);
//            } catch (Throwable e) {
...
//     * @param <K> 主键类型
//     */
//    private <T extends Serializable, K extends Serializable> void primitiveFieldDeal(ModelInfo<T, K> modelInfo,
//        Map<Class<?>, ModelInfo<? extends Serializable, ? extends Serializable>> localModelProxyIndexMap) {
//        // 实体类型
//        Class<T> entityClass = modelInfo.entityClass;
//
//        // 获取模型对象 (是否是单例, 仅取决于Model实例化工厂), 但是缓存之后就是单例的了~
//        modelInfo.model = container.getBean(ModelInstanceProvider.class).getModel(modelInfo.modelClass);
//
//        // 模型代理索引建立
...
//            fieldInfo.defaultValue = getContainer().getBean(ConversionConfig.class)
...
//     * @param <K> 主键类型
//     */
//    private <T extends Serializable, K extends Serializable> void dealPrimaryAnnotation(FieldInfo fieldInfo,
...
//                    container.getBean(IdGenerator.SnowFlakesID.class));
//            } else if (fieldInfo.javaType == String.class) {
//                if (fieldInfo.column != null) {
//                    if (fieldInfo.column.length() >= 36) {
//                        modelInfo.primaryKeyIdGenerator = ObjectUtils.typeCast(
//                            container.getBean(IdGenerator.UUID36.class));
//                    } else if (fieldInfo.column.length() >= 32) {
//                        modelInfo.primaryKeyIdGenerator = ObjectUtils.typeCast(
//                            container.getBean(IdGenerator.UUID32.class));
//                    }
//                } else {
//                    modelInfo.primaryKeyIdGenerator = ObjectUtils.typeCast(container.getBean(IdGenerator.UUID32.class));
//                }
//            } else {
//                modelInfo.primaryKeyIdGenerator = ObjectUtils.typeCast(container.getBean(IdGenerator.Never.class));
//            }
//        } else {
//            modelInfo.primaryKeyIdGenerator = ObjectUtils.typeCast(container.getBean(primary.idGenerator()));
//        }
//    }
//
//    /**
...
//     * @param <K> 主键类型
//     */
//    private <T extends Serializable, K extends Serializable> void relationFieldDeal(ModelInfo<T, K> modelInfo) {
//
//        Class<T> entityClass = modelInfo.entityClass;
//        List<Field> fields = EntityUtils.getDeclaredFieldsContainParent(entityClass);
//
...
//                    relationFieldInfo.relationSubQuery = new BelongsToQueryRelation(field, this, modelInfo.model);
//                } else if (field.isAnnotationPresent(BelongsToMany.class)) {
//                    relationFieldInfo.relationSubQuery = new BelongsToManyQueryRelation(field, this, modelInfo.model);
//                } else if (field.isAnnotationPresent(HasOneOrMany.class)) {
//                    relationFieldInfo.relationSubQuery = new HasOneOrManyQueryRelation(field, this, modelInfo.model);
//                } else {
//                    continue;
//                }
//                // 关联关系记录
