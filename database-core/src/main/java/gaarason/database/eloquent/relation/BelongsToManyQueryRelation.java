...
public class BelongsToManyQueryRelation extends BaseRelationSubQuery {
    ...
    public String[] prepareSqlArr(List<Map<String, Column>> stringColumnMapList,
        ...
        return new String[]{
            generateSqlPart.execute(ObjectUtils.typeCast(belongsToManyTemplate.targetModel.newQuery())).toSql(
                SqlType.SUB_QUERY), belongsToManyTemplate.relationModel.newQuery()
            .whereIn(belongsToManyTemplate.foreignKeyForLocalModel,
                getColumnInMapList(stringColumnMapList, belongsToManyTemplate.localModelLocalKey)).toSql(
            SqlType.SELECT)};
    }
    ...
    public RecordList<?, ?> dealBatch(String sql0, RecordList<?, ?> relationRecordList) {
        ...
        for (Record<?, ?> targetRecord : targetRecordList) {
            Object targetKey = targetRecord.getMetadataMap().get(belongsToManyTemplate.targetModelLocalKey).getValue();
    ...
    public List<? extends Serializable> filterBatchRecord(Record<?, ?> theRecord, RecordList<?, ?> targetRecordList,
        ...
        // 本表的关系键值
        Object localModelLocalKeyValue = theRecord.getMetadataMap()
            .get(belongsToManyTemplate.localModelLocalKey)
            .getValue();
    ...
    protected int attachWithTargetModelLocalKeyValues(Record<?, ?> theRecord,
        ...
        if (checkAlreadyExist) {
            ...
            List<Object> alreadyExistTargetModelLocalKeyValueList = belongsToManyTemplate.relationModel.newQuery()
                ...
                .get()
                .toList(recordTemp -> recordTemp.getMetadataMap()
                    .get(belongsToManyTemplate.foreignKeyForTargetModel)
                    .getValue());
    ...
    class BelongsToManyTemplate {
        ...
        BelongsToManyTemplate(Field field) {
            ...
            localModelLocalKey = "".equals(belongsToMany.localModelLocalKey())
                ? getPrimaryKeyColumnName(localModel)
            ...
            targetModelLocalKey = "".equals(belongsToMany.targetModelLocalKey())
                ? getPrimaryKeyColumnName(targetModel)
