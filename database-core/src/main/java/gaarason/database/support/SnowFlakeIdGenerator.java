    ...
    private void informationReport() {
        long effectiveAge =
            ((1L << (64 - 1 - SEQUENCE_BITS - WORKER_ID_BITS - DATA_CENTER_ID_BITS)) -
                (getSystemCurrentTimeMillis() - INITIAL_TIME_STAMP)) / (1000L * 3600 * 24 * 365);
        LOGGER.info(
            "雪花算法信息 : 尚可使用 " + effectiveAge + " 年, 当前 workerID " + workerId + " (最大 : " + MAX_WORKER_ID +
                ") , 当前 dataCenterID " + dataCenterId);
    }
    ...
    public synchronized Long nextId() {
        ...
        if (currentTimestamp < lastTimestamp) {
            ...
            if (offset <= MAX_BACKWARD_MS) {
                ...
                throw new SnowFlakeIdGeneratorException(
                    "当前时间 " + currentTimestamp + " 小于上一次记录的时间戳 " + lastTimestamp + " !");
            }
        }
